from ..components_YouXiBuJian import info_XinXi, player_ZhuJue
import pygame
...
class Level:
    ...
    def update(self,surface, keys):
        ...
        if self.player.dead:
            if self.current_timer-self.player.death_timer > 1000:
                self.finished = True
        else:
    ...
    def update_game_window(self,keys):
        th = self.game_window.x  + self.game_window.width
        if self.player.x_vel > 0 and self.player.rect.centerx <= th and self.game_window.right < self.end_x:
